...
def compute_self_distillation_loss(
    ...
    Args:
        student_log_probs: Log probabilities from student (current policy on original input).
        teacher_log_probs: Log probabilities from teacher (policy on reprompted input).
        response_mask: Mask for valid response tokens.
        self_distillation_config: Configuration for self-distillation.
        old_log_probs: Log probabilities from old policy (for IS weighting).
        student_all_log_probs: Full vocabulary log probs from student (for full-logit distillation).
        teacher_all_log_probs: Full vocabulary log probs from teacher (for full-logit distillation).
        student_topk_log_probs: Top-k log probs from student.
        teacher_topk_log_probs: Top-k log probs from teacher.
        self_distillation_mask: Mask for samples to apply distillation to.
        loss_agg_mode: Loss aggregation mode.
        rollout_is_weights: Rollout correction weights.
    ...
    if self_distillation_config.full_logit_distillation:
        ...
        if self_distillation_config.alpha == 0.0:
            ...
            # Compute the log of the mixture distribution for JSD
            alpha = torch.tensor(
